# ==============================================================================
# LIVE Pipeline Tests for azschooldata
# ==============================================================================
#
...
test_that("Arizona DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Arizona DOE URL and verify HTTP 200")
})
...
test_that("Can download Arizona enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Arizona enrollment file", {
  skip_if_offline()
...
test_that("azschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- azschooldata::get_available_years()
  ...
  tryCatch({
    raw <- azschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- azschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- azschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- azschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- azschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- azschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- azschooldata:::get_cache_path(2024, "enrollment")
