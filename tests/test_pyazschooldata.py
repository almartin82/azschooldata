"""
Tests for pyazschooldata Python wrapper.
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pyazschooldata as az
        assert hasattr(az, 'fetch_enr')
        assert hasattr(az, 'fetch_enr_multi')
        assert hasattr(az, 'tidy_enr')
        assert hasattr(az, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pyazschooldata as az
        assert hasattr(az, '__version__')
        assert isinstance(az.__version__, str)
    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_district' in df.columns and 'grade_level' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # Arizona should have ~1.1 million students
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        import pyazschooldata as az
        max_year = get_test_years()['max_year']
        df = az.fetch_enr_multi([max_year - 1, max_year])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_more_rows_than_single_year(self):
        ...
        import pyazschooldata as az
        max_year = get_test_years()['max_year']
        df_single = az.fetch_enr(max_year)
        df_multi = az.fetch_enr_multi([max_year - 1, max_year])
...
class TestDataIntegrity:
    ...
    def test_consistent_between_single_and_multi(self):
        ...
        max_year = get_test_years()['max_year']
        df_single = az.fetch_enr(max_year)
        df_multi = az.fetch_enr_multi([max_year])
    ...
    def test_years_within_available_range(self):
        ...
        df = az.fetch_enr(years['max_year'])
        assert len(df) > 0
